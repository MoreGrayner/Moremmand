...
class ExamplePlugin : JavaPlugin() {
    override fun onEnable() {
        // No 'this.' needed! Just use moremmand directly
        moremmand("tp") target Player run {
            val target = args.getPlayer(0) hint "대상 플레이어" to server.onlinePlayers
            success("텔레포트!")
            true
        }

        moremmand("world") run {
            val world = args.getWorld(0) hint "월드" to server.worlds
            success("월드 이동!")
            true
        }

        moremmand("give") run {
            val item = args.getMaterial(0) hint "아이템" to org.bukkit.Material.values().filter { it.isItem }
            success("아이템 지급!")
            true
        }

        moremmand("gamemode") run {
            val mode = args.getString(0) hint "게임모드" to listOf("survival", "creative", "adventure", "spectator")
            success("게임모드 변경!")
            true
        }

        moremmand("goto") run {
            val target = args.getPlayer(0) hint "플레이어" to server.onlinePlayers.first()
            success("이동!")
            true
        }

        moremmand("msg") run {
            val target = args.getPlayer(0) hint "플레이어" to org.bukkit.entity.Player::class.java
            success("메시지 전송!")
            true
        }

        moremmand("warp") run {
            val warp = args.getString(0) hint "워프 지점" to { sender: CommandSender ->
                config.getStringList("warps")
            }
            success("워프 이동!")
            true
        }

        moremmand("target") target Player run {
            val entity = args.getString(0) hint "엔티티" to player!!.world.entities.filterIsInstance<org.bukkit.entity.Monster>()
            success("타겟 지정!")
            true
        }

        moremmand("select") run {
            val option = args.getString(0) hint "옵션" to listOf("player1", "world1", "option1")
            success("선택 완료!")
            true
        }

        moremmand("mod")
            permission anyOf(listOf("mod.use", "admin.use")) run {
            success("모더레이터 명령어!")
            true
        }

        moremmand("admin")
            permission allOf(listOf("admin.use", "admin.manage")) run {
            success("관리자 명령어!")
            true
        }
    }
